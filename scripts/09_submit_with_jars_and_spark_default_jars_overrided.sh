...
docker run -it --rm \
 ...
 -v ${HOME_DIR}/lib_managed/jars/com.typesafe.play/play-json_2.11/play-json_2.11-2.6.9.jar:/workspace/play-json_2.11-2.6.9.jar \
 ...
 -u root \
 jupyter/pyspark-notebook bash -x -c " \
   echo \"spark.jars /workspace/play-json_2.11-2.6.9.jar\" > /usr/local/spark/conf/spark-defaults.conf && \
   cat /usr/local/spark/conf/spark-defaults.conf && \
   /usr/local/spark/bin/spark-submit -v \
     --master spark://spark-master:7077 \
     --name ${FILENAME} \
     --jars /workspace/play-functional_2.11-2.6.9.jar \
